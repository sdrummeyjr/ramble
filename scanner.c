...
// to read the next character, use this which consumes the current character and returns it
static char advance() {
    scanner.current++;
    return scanner.current[-1];
}

// returns the current character, but doesnt consume it
static char peek() {
...
// if the current character is the desired one, advance and return true
    ...
    for (;;) {
        ...
        switch (c) {
            ...
            case '\n':
                ...
                advance();
                break;
            case '/': // todo - change comment from // to something else
                if (peekNext() == '/') {
                    while (peek() != '\n' && !isAtEnd()) advance();
                } else {
                    return;
...
// todo READ 16.4 Identifiers and Keywords again in more detail
static TokenType identifierType() {
    switch (scanner.start[0]) {
        ...
        case 'f':
            if (scanner.current - scanner.start > 1) {
                switch (scanner.start[1]) {
                    ...
                    case 'u':
                        return checkKeyword(2,2,"nc", TOKEN_FUNC); // todo - func instead of fun
                }
            }
            break;
        ...
        case 't':
            ...
            break;
        case 'v': return checkKeyword(1, 2, "ar", TOKEN_VAR); // consider changing to let and const??
...
Token scanToken() {
    ...
    char c = advance();
    if (isAlpha(c)) return identifier(); // id identifiers and keywords
    if (isDigit(c)) return number(); // id numbers

    // read the next character from the source code then do a straightforward switch to see if it matches any
